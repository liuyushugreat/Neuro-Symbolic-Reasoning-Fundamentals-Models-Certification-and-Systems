...
**Related chapters:** Ch. 15 (Conformal Prediction and Uncertainty Calibration)

This lab shows how to wrap risk-classifier outputs with a conformal prediction layer, turning Chapter 15 concepts — nonconformity scores, prediction sets, coverage — into executable code.

## Design

- **quantile()** — calibration threshold $\hat{q}$ from nonconformity scores
- **build_prediction_set()** — build a prediction set given $\hat{q}$
- **empirical_coverage()** — empirical coverage on a test stream
- **main** — calibration set, test set, sets, and coverage printout

## Key concepts

1. Uncertainty calibration
2. Conformal prediction
3. Nonconformity scores: $s = 1 - p(y \mid x)$
4. Prediction sets
